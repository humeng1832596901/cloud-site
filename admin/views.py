...
from helper.pwd_serializer import dumps_data,loads_data
import threading
        ...
        if info:
            ...
            if user:
                ...
                if user.email:
                    email = user.email
                    #生成一个校验令牌
                    token = dumps_data(user.id)
                    #去掉token首部的b'和尾部的'
                    urls = f'{WEB_URL}/admin/findPwd/?key={token}'
                    #urls = f'{WEB_URL}/admin/findPwd/?key={user.id}?token={token}'
