...
from util.creat_hash import creat_hash
from admin.login import check_login
from util.sendemail import SendEmail
from util.pwd_serializer import dumps_data,loads_data
import threading
...
# 在config.py 中可配置 is_register 是false时不允许注册， 目前新创建用户不会有任何权限
...
class ChangeInfo(MethodView):
    def post(self):
        ...
        if show_name:
            ...
            if len(email) < 50:
                user = db.session.query(Users).filter(Users.id == user_id).one_or_none()
                keys = creat_hash(str(time.perf_counter()))
                user.share_key = keys
                db.session.commit()
...
class FindPwd(MethodView):
    def get(self):
        ...
        #user_id = loads_data(bytes(token,encoding='utf-8'))
        load = loads_data(bytes(token,encoding='utf-8'))
        if load==-1:
            return '已过期或链接不全'
        if len(load) != 1:
            return '参数错误，请确认链接是否完整'
        #user_id = list(load.keys())[0]
        l = list(load.keys())
        if len(l) != 1:
            return '参数错误，请确认链接是否完整'
        user_id = l[0]
        if user_id:
            ...
            if user:
                if user.share_key != load[user_id]:
                    return '参数错误，请确认链接是否完整'
    ...
    def post(self):
        info = request.form.get('name').split()
        if info:
            name = info[0]
            ...
            if user:
                """
                if user.sync_key:
                    #密保问题成功回答后密码重置为666666
                    if len(info) > 1:
                        s_key = ""
                        for i in range(1,len(info)):
                            if i!=len(info)-1:
                                s_key = info[i] + " "
                        if s_key==user.sync_key:
                            new_hash_pwd = creat_hash("666666")
                            db.session.update(user,{'password':new_hash_pwd})
                            db.session.commit()
                            return jsonify({'status': 'ok', 'msg': f'重置成功'})
                """
                if user.email:
                    email = user.email
                    #生成一个校验令牌并转为字符串类型
                    ...
                    user.share_key = t
                    token = dumps_data({user.id:t}).decode()
                    urls = f'{WEB_URL}/admin/findPwd/?key={token}'
                    body = f'您正在使用云盘提供的服务，点击此链接后您的账户密码将会重置为666666,如您没有进行此操作，请忽略此邮件：　{urls}'
                    user.register_key = 'ok'
                    db.session.commit()
                    try:
                        em = SendEmail()
                        #threading.Thread(target=em.Send, args=(email, 'yun cluod', body)).start()
                        em.Send(email, 'yun cloud', body)
                        return jsonify({'status': 'ok', 'msg': '请前往邮箱点击链接完成密码重置'})
                    except Exception as e:
